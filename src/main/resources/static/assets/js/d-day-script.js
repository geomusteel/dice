...
messageContainer.innerHTML = '<div class="text-white h3">D-Day를 입력해 주세요.</div>';
...
const counterMake = function (data) {
    ...
    if (remaining <= 0) {
        container.style.display = 'none';
        messageContainer.innerHTML = '<div class="text-white h3">타이머가 종료되었습니다.</div>';
        messageContainer.style.display = 'block';
        setClearInterval();
        return;
    } else if (isNaN(remaining)) {
        container.style.display = 'none';
        messageContainer.innerHTML = '<div class="text-white h3">유효한 시간대가 아닙니다.</div>';
        messageContainer.style.display = 'block';
        setClearInterval();
        return;
    }
